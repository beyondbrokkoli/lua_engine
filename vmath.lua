...
function vmath.lookAt(eye_x, eye_y, eye_z, center_x, center_y, center_z, out_mat)
    ...
    -- Gimbal Lock Prevention (Internal Up Vector)
    ...
    out_mat.m[0] = temp_r.x;  out_mat.m[4] = temp_r.y;  out_mat.m[8]  = temp_r.z;  out_mat.m[12] = -(temp_r.x*eye_x + temp_r.y*eye_y + temp_r.z*eye_z)
    out_mat.m[1] = temp_u.x;  out_mat.m[5] = temp_u.y;  out_mat.m[9]  = temp_u.z;  out_mat.m[13] = -(temp_u.x*eye_x + temp_u.y*eye_y + temp_u.z*eye_z)
    out_mat.m[2] = -temp_f.x; out_mat.m[6] = -temp_f.y; out_mat.m[10] = -temp_f.z; out_mat.m[14] = (temp_f.x*eye_x + temp_f.y*eye_y + temp_f.z*eye_z)
